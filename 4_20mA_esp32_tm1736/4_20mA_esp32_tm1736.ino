...
void loop() {
  ...
  // แปลงค่า ADC เป็นแรงดัน (ADS1115 มีความละเอียด 16-bit และ Vref = 5.0V)
  voltage = (adc0 * (5.0 / 32767.0)) + 0.301;
  ...
  Serial.print(" V  Current: "); Serial.print(current, 4);
  Serial.println(" mA");

  // ตรวจสอบแรงดัน หากน้อยกว่า 0.8V ให้แสดง "F01"
  if (voltage < 0.99) {
    display.showNumberDec(0x00, false, 3, 4);
  } else {
